...
chbw_decision:
	...
	/* Don't trigger RFK if AP doesn't update beacon from hostapd in ECSA flow */
	if (!IS_CH_WAITING(adapter_to_rfctl(padapter)) &&
	    !IS_ECSA_WAIT_UPT_BCN(rfctl)) {
		doiqk = _TRUE;
...
static void ap_process_csa(struct _ADAPTER *a)
{
	...
	if (rfctl->ap_csa_en == AP_SWITCH_CH_CSA) {
		...
		bcn_control_cmd(a, 0);
		rfctl->ap_csa_wait_update_bcn = CSA_WAIT_UPT_BCN;
...
static u8 update_csa_ie(struct _ADAPTER *a)
{
	...
	if (rfctl->ap_csa_en == AP_CSA_DISABLE || rfctl->ap_csa_remove_ie)
		return _FALSE;
