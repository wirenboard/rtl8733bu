...
void rtw_ap_createbss_ecsa_hdl(struct _ADAPTER *a, struct createbss_parm *parm)
{
	...
	if (check_ap_ecsa_state(rfctl, AP_SWITCH_CH_CSA)) {
		/* Switch channel done, not update beacon from hostapd yet */
		if (rfctl->ap_csa_wait_update_bcn == CSA_WAIT_UPT_BCN) {
			u8 enable = 0;

			rfctl->ap_csa_wait_update_bcn = CSA_WAIT_BCN_DONE;
			rtw_hal_set_hwreg(a, HW_VAR_BCN_EARLY_C2H_RPT, &enable);
			RTW_INFO("CSA : Wait beacon update after switch channel\n");

		/* Update beacon from hostap done, then update RA mask of clients */
		} else if (rfctl->ap_csa_wait_update_bcn == CSA_WAIT_BCN_DONE) {
			...
			RTW_INFO("CSA : Update beacon done, then update clients ramask\n");

		}/* Other case doesn't update beacon from hostapd */
	} else {
		u8 enable = 0;
		rtw_hal_set_hwreg(a, HW_VAR_BCN_EARLY_C2H_RPT, &enable);
		...
		RTW_INFO("CSA : Switch done, then update clients ramask\n");
	}
}

bool check_ap_ecsa_state(struct rf_ctl_t *rfctl, u8 state)
{
#ifdef DBG_CSA
	RTW_INFO("CSA : ap_csa_en = %u, check_state = %u\n", rfctl->ap_csa_en, state);
#endif
	if (rfctl->ap_csa_en == state)
		return true;
	return false;
}
...
u8 rtw_csa_switch_hdl(struct dvobj_priv *dvobj, u8 need_discon)
{
	struct rf_ctl_t *rfctl = dvobj_to_rfctl(dvobj);
	...
	if (!ifbmp_s) {
		RTW_WARN("CSA : STA disconnected; there is no need to switch channel\n");
		return _FAIL;
	}
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (need_discon) {
			...
			if (sta_iface->mlmepriv.cur_network_scanned)
				sta_iface->mlmepriv.cur_network_scanned->network.Configuration.DSConfig = req_ch;
