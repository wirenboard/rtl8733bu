...
int proc_get_pattern_info(struct seq_file *m, void *v)
{
	...
	rtw_wow_pattern_sw_dump(m, padapter);
			      ...
			      size_t count, loff_t *pos, void *data)
{
	...
	u8 tmp[MAX_PATTERN_NAME_LEN + 1 + MAX_WKFM_PATTERN_STR_LEN] = { 0 };
	int ret = count;
	s8 index = -1;
	u8 num = 0;
	...
	if (strncmp(tmp, "clean", 5) == 0) {
		for (index = 0; index < MAX_WKFM_CAM_NUM; index++) {
		...
		pwrpriv->wowlan_pattern_idx = 0;
		return count;
	} else if (strncmp(tmp, "default", 7) == 0) {
		rtw_wow_pattern_sw_reset(padapter);
		return count;
	}

	pattern_str = rtw_zvmalloc(sizeof(tmp));
	id_str = rtw_zvmalloc(sizeof(tmp));
	if (!pattern_str || !id_str) {
		ret = -ENOMEM;
		goto exit;
	}

	num = sscanf(tmp, "%s %s", id_str, pattern_str);
	/* old style pattern input */
	if (num == 1 && strchr(id_str, ':') != NULL) {
		_rtw_memcpy(pattern_str, id_str, sizeof(tmp));
		*id_str = '\0';
		index = pwrpriv->wowlan_pattern_idx;
		ret = rtw_wowlan_parser_pattern_cmd(pattern_str,
	...
	if (strlen(id_str) > 0) {
		u8 idscan_num = 0;
		idscan_num = sscanf(id_str, "%hhd", &index);
		if (idscan_num != 1) { /* this is a pattern name */
			pattern_name = id_str;
			index = rtw_find_wow_pattern_by_name(pwrpriv, pattern_name);
			if (index < 0) {
				if (num == 1) {
					/* Cannot find name for deleting */
					goto exit;
				} else { /* new entry */
					index = pwrpriv->wowlan_pattern_idx;
					_rtw_memcpy(pwrpriv->patterns[index].name,
						    pattern_name,
						    sizeof(pwrpriv->patterns[index].name));
				}
			}
		} else if (index >= 0 && index <= pwrpriv->wowlan_pattern_idx) {
			if (num == 2 && index == pwrpriv->wowlan_pattern_idx)
				rtw_sprintf(pwrpriv->patterns[index].name,
					    sizeof(pwrpriv->patterns[index].name),
					    "noname_%08x", rtw_random32());
		} else {
			RTW_INFO("Invalid pattern str: %s index %d\n", id_str, index);
			ret = -EFAULT;
			goto exit;
		}
	}

	if (num == 1) { /* delete pattern */
		rtw_delete_wow_pattern_by_index(pwrpriv, index);
		goto exit;
	} else { /* add or modify pattern info with new pattern style */
		ret = rtw_wowlan_parser_pattern_cmd(pattern_str,
						    pwrpriv->patterns[index].content,
						    &pwrpriv->patterns[index].len,
						    pwrpriv->patterns[index].mask);
...
exit:
	if (pattern_str)
		rtw_vmfree(pattern_str, sizeof(tmp));
	if (id_str)
		rtw_vmfree(id_str, sizeof(tmp));
	return ret;
}
...
void rtw_parse_mdns_domain_name(struct seq_file *m, u8 *buf, u16 buf_len, u16 cur_offset, u8 *name_len)
{
	u8 name[255] = {0};
					 ...
					 size_t count, loff_t *pos, void *data)
{
	...
	if ((num < 1) || num == 2 || (resp_idx > (MAX_MDNS_RESP_NUM - 1))) {
		...
		rtw_parse_mdns_domain_name(NULL, resp_entry->content,
...
int proc_get_wow_mdns_match_criteria(struct seq_file *m, void *v)
{
	...
	for (resp_idx = 0; resp_idx < MAX_MDNS_RESP_NUM; resp_idx++) {
		...
		for (i = 0; i < resp_entry->match_ct_num; i++) {
			...
			rtw_parse_mdns_domain_name(m, resp_entry->content,
						   resp_entry->content_len,
						   resp_entry->match_ct[i].name_offset,
						   NULL);
		}
					...
					size_t count, loff_t *pos, void *data)
{
	...
	if (strncmp(tmp, "clear", 5) == 0) {
		...
		rtw_parse_mdns_domain_name(NULL, tmp, name_buf_len, 0, &name_len);
...
int proc_get_wow_mdns_passthru_list(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < passthru_list->passthru_name_num; i++) {
		...
		rtw_parse_mdns_domain_name(m, passthru_list->passthru_name[i].name,
...
ssize_t proc_set_igi_offset(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	_adapter *pri_adapter = GET_PRIMARY_ADAPTER(adapter);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num = sscanf(tmp, "%d", &igi_offset);
		...
		if (hal->igi_offset != igi_offset)
			hal->igi_offset = igi_offset;
	}

	return count;
}

int proc_get_igi_offset(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	HAL_DATA_TYPE *hal = GET_HAL_DATA(adapter);
