...
static void rtw_parsing_ap_info(_adapter *adapter, u8 *pframe, u32 len)
{
	...
	if (elems.country_info)
		parsing_country_element_ie(&adapter->mlmeextpriv, elems.country_info - 2 , elems.country_info_len);
	/* parsing AP Channel Report */
	if (elems.ap_channel_rpt)
		parsing_ap_channel_rpt_ie(&adapter->mlmeextpriv, elems.ap_channel_rpt - 2 , elems.ap_channel_rpt_len);
}
...
void dump_mgntframe(_adapter *padapter, struct xmit_frame *pmgntframe)
{
	...
	if (RTW_CANNOT_RUN(padapter) || IS_WOW_CANT_TX(pwrpriv, pmgntframe)) {
...
void mlmeext_joinbss_event_callback(_adapter *padapter, int join_res)
{
	...
	if (join_res < 0) {
		u8 pre_igi = 0, cur_igi = 0;
		bool is_pause_dig = false;
		...
		/* enable DIG and restore IGI for assoc fail  */
		rtw_hal_get_odm_var(padapter, HAL_ODM_INITIAL_GAIN, &pre_igi, &is_pause_dig);
		if (is_pause_dig) {
			cur_igi = 0xff;
			rtw_hal_set_odm_var(padapter, HAL_ODM_INITIAL_GAIN, &cur_igi, _FALSE);
			rtw_hal_get_odm_var(padapter, HAL_ODM_INITIAL_GAIN, &cur_igi, NULL);
			RTW_INFO("%s restore IGI from 0x%02x to 0x%02x\n", __func__, pre_igi, cur_igi);
		}
...
exit_mlmeext_joinbss_event_callback:
	...
	/* enable DIG and restore IGI for non-Encryption  */
	if (psecuritypriv->ndisencryptstatus == Ndis802_11EncryptionDisabled) {
		u8 pre_igi = 0, cur_igi = 0;
		bool is_pause_dig = false;
		rtw_hal_get_odm_var(padapter, HAL_ODM_INITIAL_GAIN, &pre_igi, &is_pause_dig);
		if (is_pause_dig) {
...
#ifdef CONFIG_ECSA
	if (IS_ECSA_RUNNING(padapter) && IS_ECSA_STOP_TX(adapter_to_rfctl(padapter))) {
		RTW_INFO("CSA : don't trigger SA query timeout under the period of CSA stop TX\n");
		return;
	}
#endif
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE &&
...
u8 createbss_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if ((parm->req_ch == 0 && pmlmeinfo->state == WIFI_FW_AP_STATE)
		...
		if (IS_ECSA_RUNNING(padapter))
			rtw_ap_createbss_ecsa_hdl(padapter, parm);
#endif /* CONFIG_ECSA */
...
u8 join_cmd_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	cur_igi = rtw_phydm_get_igi_by_rssi(padapter, (s32)pnetwork->Rssi, GET_HAL_DATA(padapter)->igi_offset);
	if (cur_igi != 0) {
		rtw_hal_get_odm_var(padapter, HAL_ODM_INITIAL_GAIN, &pre_igi, NULL);
		rtw_hal_set_odm_var(padapter, HAL_ODM_INITIAL_GAIN, &cur_igi, _FALSE);
		rtw_hal_get_odm_var(padapter, HAL_ODM_INITIAL_GAIN, &cur_igi, NULL);
		RTW_INFO("%s rssi:%d dBm, set IGI from 0x%02x to 0x%02x\n", __func__,
			(int)pnetwork->Rssi, pre_igi, cur_igi);
	}
...
static void sitesurvey_res_reset(_adapter *adapter, struct sitesurvey_parm *parm)
{
	...
	if (parm->reason == RTW_AUTO_SCAN_REASON_ROAM ||
		...
		rtw_scan_ch_sort(adapter, ss->ch, ss->ch_num);
#endif
	for (i = 0; i < chset->chs_len; i++)
...
u8 set_stakey_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if (MLME_IS_STA(padapter) && !pparm->gk) {
		/* enable DIG and restore IGI for eapol end  */
		u8 pre_igi = 0, cur_igi = 0;
		bool is_pause_dig = false;
		rtw_hal_get_odm_var(padapter, HAL_ODM_INITIAL_GAIN, &pre_igi, &is_pause_dig);
		if (is_pause_dig) {
			...
			if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
				...
				if (is_grouped == _FALSE) {
					...
					if (MLME_IS_MESH(iface))
						mlmeext->cur_bwmode = rtw_mesh_adjust_2g_bw(mlmeext->cur_channel, mlmeext->cur_bwmode, mlmeext->cur_ch_offset);
					#endif
...
u8 set_csa_hdl(_adapter *adapter, unsigned char *pbuf)
{
	...
	if (rfctl->csa_ch) {
		...
		if (rtw_csa_switch_hdl(adapter_to_dvobj(adapter), param->need_disconnect) == _FAIL)
			reset_all_ecsa_param(adapter);
	}
#endif
	return H2C_SUCCESS;
}
