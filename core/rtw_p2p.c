...
int rtw_p2p_check_frames_realloc_buf(_adapter *padapter, const u8 **pbuf, u8 category, u32 len, u8 tx)
{
	const u8 *buf;
	u8 *dump_buf = NULL;
	int ret = -1;
	...
	buf = *pbuf;
	if (buf && category == RTW_WLAN_CATEGORY_PUBLIC) {
		/* Reallocate buf only if category is public action frame
		   due to buf content will only be modified by
		   rtw_p2p_check_frames when the category is public action */
		dump_buf = (u8 *)rtw_malloc(len);
		if (dump_buf != NULL) {
			_rtw_memcpy(dump_buf, (void *)buf, len);
			*pbuf = dump_buf;
		}
	}

	ret = rtw_p2p_check_frames(padapter, *pbuf, len, tx);

	if ((ret < 0) && (dump_buf != NULL)) {
		rtw_mfree((u8 *)dump_buf, len);
		*pbuf = buf;
	}
