...
void rtw_set_gpio_dft_lvl(PADAPTER padapter)
{
	struct pwrctrl_priv *pwrctrlpriv = adapter_to_pwrctl(padapter);
	u8 gpio_dft_lvl;

	if (pwrctrlpriv->wakeup_gpio_dft_lvl_policy == 1)
		gpio_dft_lvl = GPIO_OUTPUT_LOW;
	else
		gpio_dft_lvl = (pwrctrlpriv->is_high_active == 0) ? GPIO_OUTPUT_HIGH : GPIO_OUTPUT_LOW;

	pwrctrlpriv->wowlan_gpio_output_state = gpio_dft_lvl;
}
...
#define LPS_1T1R_ARG
	...
	pwrctrlpriv->wakeup_gpio_dft_lvl_policy = CONFIG_WAKEUP_GPIO_DFT_LVL_POLICY;
	...
	RTW_INFO("%s: set GPIO_%d to OUTPUT %s state in initial and %s_ACTIVE.\n",
		 ...
		 pwrctrlpriv->is_high_active ? "HIGH" : "LOW");
