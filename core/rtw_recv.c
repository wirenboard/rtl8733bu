	...
	*sta = rtw_get_stainfo(stapriv, rattrib->ta);
	if (*sta == NULL) {
		...
		if (!is_ra_bmc && !IS_RADAR_DETECTED(adapter_to_rfctl(adapter))) {
			if (MLME_IS_LINKING(adapter) == _TRUE &&
			    _rtw_memcmp(rattrib->ta, mybssid, ETH_ALEN) == _TRUE) {
				RTW_INFO(FUNC_ADPT_FMT" drop this packet due to under linking with TA: "MAC_FMT"\n"
...
static int rtw_recv_indicatepkt_check(union recv_frame *rframe, u8 *ehdr_pos, u32 pkt_len)
{
	...
	if (ntohs(ehdr->h_proto) == ETH_P_IP) {
		...
		if (GET_IPV4_PROTOCOL(ip) == 0x11) { /* UDP */
			u8 *udp_hdr = ip + 20;
			u16 remain_pkt_len = pkt_len - ETH_HLEN - IPV4_HDR_LEN - UDP_HDR_LEN;
			if (is_dhcp_pkt(udp_hdr, remain_pkt_len))
				parsing_dhcp_packet(udp_hdr + UDP_HDR_LEN, remain_pkt_len, 0);
		}
	}
#endif
