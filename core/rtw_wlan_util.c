...
int rtw_check_bcn_info(ADAPTER *Adapter, u8 *pframe, u32 packet_len)
{
	...
	if ((new_bcn_max != 0) && (pmlmepriv->new_beacon_cnts >= new_bcn_max))
#else
...
bool is_dhcp_pkt(u8 *udp_hdr, u16 remain_pkt_len)
{
	u16 udp_payload_len;

	/* 67 : DHCP server, 68 : DHCP client */
	if ((GET_UDP_SRC(udp_hdr) == 68 && GET_UDP_DST(udp_hdr) == 67) ||
	    (GET_UDP_SRC(udp_hdr) == 67 && GET_UDP_DST(udp_hdr) == 68)) {
		udp_payload_len = GET_UDP_LEN(udp_hdr) - UDP_HDR_LEN;
		if (udp_payload_len >= MINIMUM_DHCP_PAYLOAD_SIZE &&
		    remain_pkt_len >= MINIMUM_DHCP_PAYLOAD_SIZE)
			return true;
	}
	return false;
}
...
#ifdef DBG_DHCP_DUMP
static void dump_dhcp_type(u8 dhcp_msg_type, u8 trx_type)
{
	char *trx_msg = trx_type ? "send" : "recv";

	switch (dhcp_msg_type) {
	case 1:
		RTW_INFO("%s DHCP Discovery\n", trx_msg);
		break;
	case 2:
		RTW_INFO("%s DHCP Offer\n", trx_msg);
		break;
	case 3:
		RTW_INFO("%s DHCP Request\n", trx_msg);
		break;
	case 4:
		RTW_INFO("%s DHCP Decline\n", trx_msg);
		break;
	case 5:
		RTW_INFO("%s DHCP ACK\n", trx_msg);
		break;
	case 6:
		RTW_INFO("%s DHCP NACK\n", trx_msg);
		break;
	case 7:
		RTW_INFO("%s DHCP Release\n", trx_msg);
		break;
	case 8:
		RTW_INFO("%s DHCP Inform\n", trx_msg);
		break;
	default:
		RTW_INFO("%s DHCP packet(%d)\n", trx_msg, dhcp_msg_type);
	}
}

void parsing_dhcp_packet(u8 *pkt, u16 pkt_len, u8 trx_type)
{
	u8 option, len;
	u8 dhcp_msg_type;
	u8 *pos;
	...
	/*
	* pos is the pointer of DHCP options in DHCP payload.
	* left is the remain length of DHCP options.
	* The address of DHCP options is DHCP payload shift 240 bytes.
	* 236 bytes fixed lendth + 4 bytes magic cookie = shift 240 bytes
	*/
	pos = pkt + 240;
	left = pkt_len - 240;
	...
	while (left >= 2) {
		...
		if (len > left) {
			RTW_ERR("%s : parse failed (option=%u len=%u left=%u)\n",
				 __func__, option, len, left);
			break;
		}
	...
	dump_dhcp_type(dhcp_msg_type, trx_type);
}
#endif
...
void rtw_wow_pattern_sw_reset(_adapter *adapter)
{
	...
	for (i = 0 ; i < MAX_WKFM_CAM_NUM; i++) {
		...
		_rtw_memset(pwrctrlpriv->patterns[i].name, '\0', sizeof(pwrctrlpriv->patterns[i].name));
...
void rtw_set_google_cast_mdns_wow_pattern(_adapter *padapter)
{
	...
	for (i = 0; i < GOOGLE_CAST_PATTERN_NUM; i++) {
		...
		*ptr = ip_protocol;
		...
		rtw_sprintf(pwrpriv->patterns[index].name,
			    sizeof(pwrpriv->patterns[index].name),
			    "%s%d", RTL_GOOGLE_CAST_PATTERN_PREFIX, index);
		index++;
	}
	pwrpriv->wowlan_pattern_idx = index;
}
#endif

s8 rtw_find_wow_pattern_by_name(struct pwrctrl_priv *pwrpriv,
				const char *pattern_name)
{
	int i;
	for (i = 0; i < pwrpriv->wowlan_pattern_idx; i++) {
		if (!strncmp(pwrpriv->patterns[i].name, pattern_name,
			     MAX_PATTERN_NAME_LEN))
			return i;
	}
...
u8 rtw_delete_wow_pattern_by_index(struct pwrctrl_priv *pwrpriv, u8 index) {
	u8 i;
	if (index >= pwrpriv->wowlan_pattern_idx)
		return pwrpriv->wowlan_pattern_idx;

	RTW_INFO("delete pattern index: %d\n", index);
	for (i = index; i < pwrpriv->wowlan_pattern_idx - 1; i++) {
		_rtw_memcpy(pwrpriv->patterns[i].content,
			    pwrpriv->patterns[i + 1].content,
			    sizeof(pwrpriv->patterns[i + 1].content));
		_rtw_memcpy(pwrpriv->patterns[i].mask,
			    pwrpriv->patterns[i + 1].mask,
			    sizeof(pwrpriv->patterns[i + 1].mask));
		_rtw_memcpy(pwrpriv->patterns[i].name,
			    pwrpriv->patterns[i + 1].name,
			    sizeof(pwrpriv->patterns[i + 1].name));
		pwrpriv->patterns[i].len = pwrpriv->patterns[i + 1].len;
	}
	_rtw_memset(pwrpriv->patterns[pwrpriv->wowlan_pattern_idx-1].content, 0,
		    sizeof(pwrpriv->patterns[pwrpriv->wowlan_pattern_idx - 1].content));
	_rtw_memset(pwrpriv->patterns[pwrpriv->wowlan_pattern_idx-1].mask, 0,
		    sizeof(pwrpriv->patterns[pwrpriv->wowlan_pattern_idx - 1].mask));
	_rtw_memset(pwrpriv->patterns[pwrpriv->wowlan_pattern_idx-1].name, 0,
		    sizeof(pwrpriv->patterns[pwrpriv->wowlan_pattern_idx - 1].name));
	pwrpriv->patterns[pwrpriv->wowlan_pattern_idx - 1].len = 0;
	pwrpriv->wowlan_pattern_idx--;

	return pwrpriv->wowlan_pattern_idx;
}

void rtw_fill_pattern_ipmac(_adapter *adapter) {
	...
	for (index = 0; index < DEFAULT_PATTERN_NUM; index++) {
		if (strncmp(pwrpriv->patterns[index].name,
			    RTL_DEFAULT_PATTERN_PREFIX,
			    strlen(RTL_DEFAULT_PATTERN_PREFIX)) != 0)
			continue;
...
u8 rtw_set_default_pattern(_adapter *adapter)
{
	...
	for (index = 0 ; index < DEFAULT_PATTERN_NUM ; index++) {
		...
		rtw_sprintf(pwrpriv->patterns[index].name,
			    sizeof(pwrpriv->patterns[index].name),
			    "%s%d", RTL_DEFAULT_PATTERN_PREFIX, index);
	}
	pwrpriv->wowlan_pattern_idx = index;
#ifdef CONFIG_GOOGLE_CAST_WAKEUP
...
void rtw_wow_pattern_sw_dump(struct seq_file *m, _adapter *padapter)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	u8 val8;
	char str_1[128];
	...
	int i = 0 , j = 0, k = 0;
	int len = 0, max_len = 0;
	struct rtl_priv_pattern *pattern = NULL;
	...
	rtw_fill_pattern_ipmac(padapter);
	for (k = 0; k < pwrpriv->wowlan_pattern_idx ; k++) {
		pattern = &pwrpriv->patterns[k];
