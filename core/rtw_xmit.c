...
get_sta_info:
	...
	if (ETH_P_IP == pattrib->ether_type) {
		...
		if (GET_IPV4_PROTOCOL(ip) == 0x01) { /* ICMP */
			...
			u8 udp_hdr[UDP_HDR_LEN];
			u8 waveagent_payload[16];
			u16 remain_pkt_len;

			_rtw_pktfile_read(&pktfile, udp_hdr, UDP_HDR_LEN);
			remain_pkt_len = pktfile.pkt_len;

			if (is_dhcp_pkt(udp_hdr, remain_pkt_len)) {
				u8 *dhcp_payload;

				pattrib->dhcp_pkt = 1;
				DBG_COUNTER(padapter->tx_logs.core_tx_upd_attrib_dhcp);
				#ifdef DBG_DHCP_DUMP
				dhcp_payload = (u8 *)rtw_zmalloc(remain_pkt_len);
				if (dhcp_payload) {
					_rtw_pktfile_read(&pktfile, dhcp_payload, remain_pkt_len);
					parsing_dhcp_packet(dhcp_payload,
							    remain_pkt_len, 1);
					rtw_mfree(dhcp_payload, remain_pkt_len);
				}
				#endif
			}

			/* WaveAgent packet, increase priority so that the system can read data in time */
			_rtw_pktfile_read(&pktfile, waveagent_payload, 16);
			if (IS_WAVEAGENT_PKT(waveagent_payload)) {
...
bool rtw_is_xmit_blocked(_adapter *padapter)
{
	...
	_enter_critical_bh(&dvobj->xmit_block_lock, &irqL);
	is_blocked = ((dvobj->xmit_block) ? _TRUE : _FALSE);
	_exit_critical_bh(&dvobj->xmit_block_lock, &irqL);
