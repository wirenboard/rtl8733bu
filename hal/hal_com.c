...
void rtw_dump_fw_info(void *sel, _adapter *adapter)
{
	...
	if (hal_data->bFWReady)
		RTW_PRINT_SEL(sel, "FW VER -%d.%d.%d\n",
			hal_data->firmware_version,
			hal_data->firmware_sub_version,
			hal_data->firmware_sub_index);
	else
...
struct sta_info *_rtw_tx_stat_get_ongoing_sta(_adapter *adapter, bool done, const char *caller)
{
	struct sta_priv	*stapriv_primary = &(GET_PRIMARY_ADAPTER(adapter))->stapriv;
	_adapter *adapter_ognl;
	struct sta_info *sta = NULL;

	enter_critical_bh(&stapriv_primary->tx_rpt_lock);

	if (!stapriv_primary->gotc2h) {
		RTW_WARN("%s: No gotc2h!\n", caller);
		goto exit;
	}

	adapter_ognl = rtw_get_iface_by_id(adapter, stapriv_primary->c2h_adapter_id);
	if (!adapter_ognl) {
		RTW_WARN("%s: No adapter!\n", caller);
		goto done;
	}

	sta = rtw_get_stainfo(&adapter_ognl->stapriv, stapriv_primary->c2h_sta_mac);
	if (!sta) {
	...
	if (done)
		rtw_sctx_done(&stapriv_primary->gotc2h);

exit:
	exit_critical_bh(&stapriv_primary->tx_rpt_lock);
...
#endif
	pHalData->igi_offset = regsty->igi_offset;
}
...
void rtw_hal_switch_gpio_wl_ctrl(_adapter *padapter, u8 index, u8 enable)
{
	...
	/* The WAKEUP_GPIO_IDX pin mux of the 8733b chipset is automatically configured by the FW. */
...
void rtw_hal_set_mdns_ofld_cmd(PADAPTER padapter, PRSVDPAGE_LOC rsvdpageloc)
{
	...
	SET_H2CCMD_MDNS_OFFLOAD_EN(h2c_mdns_offload_parm,
				   mdns_ofld_info->offload_state);
	RTW_INFO("%s: offload_en =%d \n", __func__, mdns_ofld_info->offload_state);

	if (mdns_ofld_info->offload_state) {
		/* 0 to wakeup on Rx mdns query, but will no longer be used */
		SET_H2CCMD_MDNS_OFFLOAD_STATE(h2c_mdns_offload_parm, 1);
...
static void rtw_hal_ap_wow_disable(_adapter *padapter)
{
	...
	RTW_INFO("%s: set GPIO_%d to OUTPUT %s state in ap wow resume and %s_ACTIVE.\n",
		 ...
		 pwrctl->is_high_active ? "HIGH" : "LOW");
#endif /*CONFIG_WAKEUP_GPIO_INPUT_MODE*/
#endif /* CONFIG_RTW_ONE_PIN_GPIO */
#endif /* CONFIG_GPIO_WAKEUP */
	media_status_rpt = RT_MEDIA_CONNECT;
...
static u8 rtw_hal_wow_pattern_generate(_adapter *adapter, u8 idx, struct rtl_wow_pattern *pwow_pattern)
{
	...
	mask_len = (mask_len > MAX_WKFM_SIZE) ? MAX_WKFM_SIZE : mask_len;
...
static void rtw_hal_dl_pattern(_adapter *adapter, u8 mode)
{
	...
	case 1:
		...
		#else
		rtw_clean_pattern(adapter);
		/* Fill the IP and MAC address for the default patterns,
		as they may be changed at any time before suspension */
		rtw_fill_pattern_ipmac(adapter);
...
#ifdef CONFIG_MDNS_OFFLOAD
		if ((hal_spec->wow_cap & WOW_CAP_MDNS) && mdns_ofld_info->offload_state) {
			int i;
...
static u8 rtw_hal_set_fw_bcn_early_c2h_rpt_cmd(struct _ADAPTER *adapter, u8 enable)
{
	...
	RTW_PRINT("%s -%s()-\n", enable ? "Enable" : "Disable", __func__);
