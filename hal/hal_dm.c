	...
	void						*pValue2)
{
	...
	case HAL_ODM_INITIAL_GAIN:
		*((u8 *)pValue1) = rtw_phydm_get_cur_igi(Adapter);
		if (pValue2)
			*((bool *)pValue2) = podmpriv->is_pause_dig;
		break;
	default:
		break;
	}
	...
	for (i = 0; i < dvobj->iface_nums; i++){
		...
		if (iface!= NULL) {
			...
			if( pmlmeinfo->assoc_AP_vendor == HT_IOT_PEER_ATHEROS){
				...
				ATHEROS_HWID = TRUE;
				goto exit;
			} else if (pmlmeinfo->assoc_AP_vendor == HT_IOT_PEER_BROADCOM) {
				BROADCOM_HWID = TRUE;
				goto exit;
...
/*
 * Description:
 *	Get suitable IGI according to input RSSI value
 * Parameters:
 *	rssi:RSSI value(dbm)
 * Parameters:
 *	offset: decide by driver
 * Return:
 *	IGI value
 */
u8 rtw_phydm_get_igi_by_rssi(PADAPTER adapter, s32 rssi, s8 offset)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);

	return phydm_get_igi_for_target_pin_scan(phydm, translate_dbm_to_percentage(rssi) + offset);
}
