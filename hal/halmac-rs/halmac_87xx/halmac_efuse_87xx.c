...
efuse_cmd_cnstr_state_87xx(struct halmac_adapter *adapter);

static enum halmac_ret_status
proc_dump_efuse_87xx(struct halmac_adapter *adapter,
		     enum halmac_efuse_read_cfg cfg);

static enum halmac_ret_status
read_hw_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u32 size,
		   u8 *map);

static enum halmac_ret_status
read_log_efuse_map_87xx(struct halmac_adapter *adapter, u8 *map);

static enum halmac_ret_status
proc_pg_efuse_by_map_87xx(struct halmac_adapter *adapter,
			  struct halmac_pg_efuse_info *info,
			  enum halmac_efuse_read_cfg cfg);

static enum halmac_ret_status
dump_efuse_fw_87xx(struct halmac_adapter *adapter);

static enum halmac_ret_status
dump_efuse_drv_87xx(struct halmac_adapter *adapter);

static enum halmac_ret_status
proc_write_log_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 value);

//jerry_zhou
static enum halmac_ret_status
proc_write_log_efuse_8733B(struct halmac_adapter *adapter, u32 offset, u16 value);


static enum halmac_ret_status
update_eeprom_mask_87xx(struct halmac_adapter *adapter,
			struct halmac_pg_efuse_info *info, u8 *updated_mask);

static enum halmac_ret_status
check_efuse_enough_87xx(struct halmac_adapter *adapter,
			struct halmac_pg_efuse_info *info, u8 *updated_mask);

static enum halmac_ret_status
pg_extend_efuse_87xx(struct halmac_adapter *adapter,
		     struct halmac_pg_efuse_info *info, u8 word_en,
		     u8 pre_word_en, u32 eeprom_offset);

static enum halmac_ret_status
proc_pg_efuse_87xx(struct halmac_adapter *adapter,
		   struct halmac_pg_efuse_info *info, u8 word_en,
		   u8 pre_word_en, u32 eeprom_offset);

static enum halmac_ret_status
program_efuse_87xx(struct halmac_adapter *adapter,
		   struct halmac_pg_efuse_info *info, u8 *updated_mask);

static void
mask_eeprom_87xx(struct halmac_adapter *adapter,
		 struct halmac_pg_efuse_info *info);

/**
 * dump_efuse_map_87xx() - dump "physical" efuse map
...
dump_efuse_map_87xx(struct halmac_adapter *adapter,
		    enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	u8 *efuse_map;
	...
	if (efuse_cmd_cnstr_state_87xx(adapter) != HALMAC_CMD_CNSTR_IDLE) {
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	status = switch_efuse_bank_87xx(adapter, HALMAC_EFUSE_BANK_WIFI);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]switch efuse bank!!\n");
		return status;
	}

	status = proc_dump_efuse_87xx(adapter, cfg);
	if (status != HALMAC_RET_SUCCESS) {
	...
	if (cnv_efuse_state_87xx(adapter, HALMAC_CMD_CNSTR_IDLE) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
...
/**
 * dump_efuse_map_bt_87xx() - dump "BT physical" efuse map
 * @adapter : the adapter of halmac
 * @bank : bt efuse bank
 ...
 */
enum halmac_ret_status
dump_efuse_map_bt_87xx(struct halmac_adapter *adapter,
		       enum halmac_efuse_bank bank, u32 size, u8 *map)
{
	...
	status = switch_efuse_bank_87xx(adapter, bank);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]switch efuse bank!!\n");
		return status;
	}

	status = read_hw_efuse_87xx(adapter, 0, size, map);
	if (status != HALMAC_RET_SUCCESS) {
	...
	//jerry_zhou 20201103 change bank to wifi
	status = switch_efuse_bank_87xx(adapter, HALMAC_EFUSE_BANK_WIFI);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]switch efuse bank to wifi!!\n");
		return status;
	}
...
/**
 * write_efuse_bt_87xx() - write "BT physical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : Write value
 * @map : bt efuse map
 ...
 */
enum halmac_ret_status
write_efuse_bt_87xx(struct halmac_adapter *adapter, u32 offset, u8 value,
		    enum halmac_efuse_bank bank)
{
	...
	status = write_hw_efuse_87xx(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * read_efuse_bt_87xx() - read "BT physical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 ...
 */
enum halmac_ret_status
read_efuse_bt_87xx(struct halmac_adapter *adapter, u32 offset, u8 *value,
		   enum halmac_efuse_bank bank)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = read_efuse_87xx(adapter, offset, 1, value);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * cfg_efuse_auto_check_87xx() - check efuse after writing it
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
cfg_efuse_auto_check_87xx(struct halmac_adapter *adapter, u8 enable)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
/**
 * get_efuse_available_size_87xx() - get efuse available size
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_efuse_available_size_87xx(struct halmac_adapter *adapter, u32 *size)
{
	enum halmac_ret_status status;

	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);

	status = dump_log_efuse_map_87xx(adapter, HALMAC_EFUSE_R_DRV);
...
/**
 * get_efuse_size_87xx() - get "physical" efuse size
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_efuse_size_87xx(struct halmac_adapter *adapter, u32 *size)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
/**
 * get_log_efuse_size_87xx() - get "logical" efuse size
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_log_efuse_size_87xx(struct halmac_adapter *adapter, u32 *size)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
/**
 * dump_log_efuse_map_87xx() - dump "logical" efuse map
 * @adapter : the adapter of halmac
 * @cfg : dump efuse method
 * Author : Soar
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
dump_log_efuse_map_87xx(struct halmac_adapter *adapter,
			enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	if (adapter->efuse_map_valid == 1) {
		*proc_status = HALMAC_CMD_PROCESS_DONE;
		...
		if (eeprom_parser_87xx(adapter, adapter->efuse_map, map) !=
		    HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, size);
...
enum halmac_ret_status
dump_log_efuse_mask_87xx(struct halmac_adapter *adapter,
			 enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	u32 size = adapter->hw_cfg_info.eeprom_size;
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	if (adapter->efuse_map_valid == 1) {
		*proc_status = HALMAC_CMD_PROCESS_DONE;
		...
		if (eeprom_mask_parser_87xx(adapter, adapter->efuse_map, map) !=
		    HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, size);
			return HALMAC_RET_EEPROM_PARSING_FAIL;
		}
...
/**
 * read_logical_efuse_87xx() - read logical efuse map 1 byte
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : 1 byte efuse value
 * Author : Soar
 ...
 */
enum halmac_ret_status
read_logical_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 *value)
{
	u8 *map = NULL;
	u32 size = adapter->hw_cfg_info.eeprom_size;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	status = read_log_efuse_map_87xx(adapter, map);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * write_log_efuse_87xx() - write "logical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : value
 * Author : Soar
 ...
 */
enum halmac_ret_status
write_log_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = proc_write_log_efuse_87xx(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS) {
...
//jerry_zhou
enum halmac_ret_status
write_log_efuse_8733B(struct halmac_adapter *adapter, u32 offset, u16 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = proc_write_log_efuse_8733B(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]write logical efuse\n");
		return status;
	}
...
/**
 * pg_efuse_by_map_87xx() - pg logical efuse by map
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
pg_efuse_by_map_87xx(struct halmac_adapter *adapter,
		     struct halmac_pg_efuse_info *info,
		     enum halmac_efuse_read_cfg cfg)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	status = proc_pg_efuse_by_map_87xx(adapter, info, cfg);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * mask_log_efuse_87xx() - mask logical efuse
 * @adapter : the adapter of halmac
 * @info : efuse map information
 * Author : Soar
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
mask_log_efuse_87xx(struct halmac_adapter *adapter,
		    struct halmac_pg_efuse_info *info)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
enum halmac_ret_status
switch_efuse_bank_87xx(struct halmac_adapter *adapter,
		       enum halmac_efuse_bank bank)
{
	u8 reg_value;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;

	if (cnv_efuse_state_87xx(adapter, HALMAC_CMD_CNSTR_BUSY) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
	...
	if (bank == (reg_value & (BIT(0) | BIT(1))))
		return HALMAC_RET_SUCCESS;
	//jerry_zhou 20201103 change bank
		     ...
		     enum halmac_efuse_read_cfg cfg)
{
	...
	*proc_status = HALMAC_CMD_PROCESS_SENDING;
	...
	if (cnv_efuse_state_87xx(adapter, HALMAC_CMD_CNSTR_H2C_SENT) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;

	if (cfg == HALMAC_EFUSE_R_AUTO) {
		//h2c_init = HALMAC_REG_R32(REG_H2C_PKT_READADDR);
		//if (adapter->halmac_state.dlfw_state == HALMAC_DLFW_NONE || h2c_init == 0)

		if (adapter->halmac_state.dlfw_state == HALMAC_DLFW_NONE )
			status = dump_efuse_drv_87xx(adapter);
		else
			status = dump_efuse_fw_87xx(adapter);
...
enum halmac_ret_status
cnv_efuse_state_87xx(struct halmac_adapter *adapter,
...
enum halmac_ret_status
write_hw_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	...
	HALMAC_REG_W8(REG_PMC_DBG_CTRL2 + 3, unlock_code); //grant code for efuse burn
	/*
	value1=HALMAC_REG_R8(0x1008);
	value1=value1|BIT6;
	HALMAC_REG_W8(0x1008,value1);
	*/

	value1=HALMAC_REG_R8(REG_SYS_ISO_CTRL+1);
	value1=value1|BIT5;
	HALMAC_REG_W8(REG_SYS_ISO_CTRL+1,value1);//open small power cut for efuse 3.3V

	PLTFM_DELAY_US(1000);                    //delay 1ms

	value1=HALMAC_REG_R8(REG_SYS_ISO_CTRL+1);
	value1=value1| BIT6;
	HALMAC_REG_W8(REG_SYS_ISO_CTRL+1,value1);//open large power cut for efuse 3.3V
	...
	*/
	...
	/*
	  0x30[7:0] efuse write data
	  0x30[17:8] efuse write addr
	  0x30[31]=1 for program
	*/
	value32 = HALMAC_REG_R32(REG_EFUSE_CTRL);
	value32 &= ~(BIT_MASK_EF_DATA | BITS_EF_ADDR);
	...
	} while (BIT_EF_FLAG == (tmp32 & BIT_EF_FLAG));//polling 0x30[31] 0: write done 

	value1=HALMAC_REG_R8(REG_SYS_ISO_CTRL+1);
	value1=value1&0xBF;
	HALMAC_REG_W8(REG_SYS_ISO_CTRL+1,value1);//close large power cut for efuse 3.3V

	PLTFM_DELAY_US(1000);                    //delay 1ms

	value1=HALMAC_REG_R8(REG_SYS_ISO_CTRL+1);
	value1=value1&0xDF;
	HALMAC_REG_W8(REG_SYS_ISO_CTRL+1,value1);//close small power cut for efuse 3.3V
	...
	*/
	...
	if (adapter->efuse_auto_check_en == 1) {
		if (read_hw_efuse_87xx(adapter, offset, 1, &value_read) !=
		    HALMAC_RET_SUCCESS)
			return HALMAC_RET_EFUSE_R_FAIL;
		if (value_read != value) {
...
enum halmac_ret_status
eeprom_parser_87xx(struct halmac_adapter *adapter, u8 *phy_map, u8 *log_map)
{
	u8 i;
	u8 value8;
	u8 blk_idx;
...
enum halmac_ret_status
eeprom_mask_parser_87xx(struct halmac_adapter *adapter, u8 *phy_map,
			u8 *log_mask)
{
	u8 i;
	u8 value8;
...
read_log_efuse_map_87xx(struct halmac_adapter *adapter, u8 *map)
{
	...
	if (adapter->efuse_map_valid == 0) {
		...
		status = read_efuse_87xx(adapter, 0, efuse_size, local_map);
		if (status != HALMAC_RET_SUCCESS) {
			PLTFM_MSG_ERR("[ERR]read efuse\n");
			  ...
			  enum halmac_efuse_read_cfg cfg)
{
	...
	status = update_eeprom_mask_87xx(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
	...
	status = check_efuse_enough_87xx(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
	...
	status = program_efuse_87xx(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]pg efuse\n");
		PLTFM_FREE(updated_mask, mask_size);
		return status;
...
dump_efuse_drv_87xx(struct halmac_adapter *adapter)
{
	...
	if (!adapter->efuse_map) {
		...
		if (!adapter->efuse_map) {
			PLTFM_MSG_ERR("[ERR]malloc adapter map!!\n");
			reset_ofld_feature_87xx(adapter,
	...
	if (adapter->efuse_map_valid == 0) {
		...
		if (read_hw_efuse_87xx(adapter, 0, efuse_size, map) !=
		    HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, efuse_size);
			return HALMAC_RET_EFUSE_R_FAIL;
		}
...
proc_write_log_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	...
	if (*(map + offset) != value) {
		...
		if (offset > 0x7f) {
				...
				status = write_hw_efuse_87xx(adapter, end, hdr);
				if (status != HALMAC_RET_SUCCESS) {
					PLTFM_FREE(map, eeprom_size);
					return status;
				}

				status = write_hw_efuse_87xx(adapter, end + 1, hdr2);
				if (status != HALMAC_RET_SUCCESS) {
					PLTFM_FREE(map, eeprom_size);
					return status;
				}

				status = write_hw_efuse_87xx(adapter, end + 2, byte1);
				if (status != HALMAC_RET_SUCCESS) {
					PLTFM_FREE(map, eeprom_size);
					return status;
				}

				status = write_hw_efuse_87xx(adapter, end + 3, byte2);
				if (status != HALMAC_RET_SUCCESS) {
					PLTFM_FREE(map, eeprom_size);
					return status;
				}
				...
				status = write_hw_efuse_87xx(adapter, end + 1, byte1);
				if (status != HALMAC_RET_SUCCESS) {
					PLTFM_FREE(map, eeprom_size);
					return status;
				}
				status = write_hw_efuse_87xx(adapter, end + 2, byte2);
				if (status != HALMAC_RET_SUCCESS) {
					PLTFM_FREE(map, eeprom_size);
					return status;
				}
...
enum halmac_ret_status
read_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u32 size, u8 *map)
{
	if (!map) {
		PLTFM_MSG_ERR("[ERR]malloc map\n");
		return HALMAC_RET_NULL_POINTER;
	...
	if (adapter->efuse_map_valid == 1) {
		...
		if (read_hw_efuse_87xx(adapter, offset, size, map) !=
		    HALMAC_RET_SUCCESS)
			return HALMAC_RET_EFUSE_R_FAIL;
	}
		     ...
		     u8 pre_word_en, u32 eeprom_offset)
{
	...
	status = write_hw_efuse_87xx(adapter, efuse_end, hdr);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]write efuse\n");
		return status;
	}

	status = write_hw_efuse_87xx(adapter, efuse_end + 1, hdr2);
	if (status != HALMAC_RET_SUCCESS) {
	...
	for (i = 0; i < 4; i++) {
		if (((pre_word_en >> i) & 0x1) > 0) {
			status = write_hw_efuse_87xx(adapter, efuse_end,
						     *(info->efuse_map +
						     eeprom_offset +
						     (i << 1)));
			if (status != HALMAC_RET_SUCCESS) {
			...
			status = write_hw_efuse_87xx(adapter, efuse_end + 1,
						     *(info->efuse_map +
						     eeprom_offset + (i << 1)
						     + 1));
			if (status != HALMAC_RET_SUCCESS) {
		   ...
		   struct halmac_pg_efuse_info *info, u8 *updated_mask)
{
	...
	for (i = 0; i < info->efuse_map_size; i = i + 8) {
		...
		if (pre_word_en > 0) {
			if (eeprom_offset > 0x7f) {
				status = pg_extend_efuse_87xx(adapter, info,
							      word_en,
							      pre_word_en,
							      eeprom_offset);
				if (status != HALMAC_RET_SUCCESS) {
				...
				status = proc_pg_efuse_87xx(adapter, info,
							    word_en,
							    pre_word_en,
							    eeprom_offset);
				if (status != HALMAC_RET_SUCCESS) {
...
enum halmac_ret_status
get_efuse_data_87xx(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	u8 seg_id;
	u8 seg_size;
	u8 seq_num;
	...
	PLTFM_MEMCPY(adapter->efuse_map + seg_id * adapter->efuse_seg_size,
		     buf + C2H_DATA_OFFSET_87XX, seg_size);
	PLTFM_MUTEX_UNLOCK(&adapter->efuse_mutex);
	...
	if ((enum halmac_h2c_return_code)fw_rc == HALMAC_H2C_RETURN_SUCCESS) {
		...
		if (adapter->evnt.log_efuse_map == 1) {
			if (eeprom_parser_87xx(adapter, adapter->efuse_map,
		...
		if (adapter->evnt.log_efuse_mask == 1) {
			if (eeprom_mask_parser_87xx(adapter, adapter->efuse_map,
						    map)
						    != HALMAC_RET_SUCCESS) {
				PLTFM_FREE(map, eeprom_size);
				return HALMAC_RET_EEPROM_PARSING_FAIL;
...
enum halmac_ret_status
get_dump_phy_efuse_status_87xx(struct halmac_adapter *adapter,
			       enum halmac_cmd_process_status *proc_status,
			       u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_dump_log_efuse_status_87xx(struct halmac_adapter *adapter,
			       enum halmac_cmd_process_status *proc_status,
			       u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_dump_log_efuse_mask_status_87xx(struct halmac_adapter *adapter,
				    enum halmac_cmd_process_status *proc_status,
				    u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_h2c_ack_phy_efuse_87xx(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	u8 seq_num = 0;
	u8 fw_rc;
	struct halmac_efuse_state *state = &adapter->halmac_state.efuse_state;
...
u32
get_rsvd_efuse_size_87xx(struct halmac_adapter *adapter)
{
	return adapter->hw_cfg_info.prtct_efuse_size;
}

/**
 * write_wifi_phy_efuse_87xx() - write wifi physical efuse
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
write_wifi_phy_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
...
/**
 * read_wifi_phy_efuse_87xx() - read wifi physical efuse
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
read_wifi_phy_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u32 size,
			 u8 *value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = read_hw_efuse_87xx(adapter, offset, size, value);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]read hw efuse\n");
		return status;
	}
