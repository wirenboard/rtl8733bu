...
void phydm_bw_fixed_enable(void *dm_void, u8 enable)
{
	...
	else if (dm->support_ic_type & (ODM_RTL8822C | ODM_RTL8822E | ODM_RTL8197G | ODM_RTL8733B))
...
void phydm_gpio_ctrl_dbg(void *dm_void, char input[][16], u32 *_used,
			 char *output, u32 *_out_len)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u32 used = *_used;
...
#if (RTL8822E_SUPPORT)
		phydm_gpio_ctrl_8822e(dm, var[0], var[1]);
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "Set 8822E GPIO %d to %d\n", var[0], var[1]);
#endif
	}
