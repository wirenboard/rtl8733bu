...
/* @1 All platforms support */
	...
	if (dm->support_ic_type == ODM_RTL8703B) {
		if (config_type == CONFIG_RF_RADIO) {
			...
			READ_AND_CONFIG_MP(8703b, _txpwr_lmt);
		}
	}
#endif
...
/*@jj add 20170822*/
	...
	if (dm->support_ic_type == ODM_RTL8822C) {
		...
		if (dm->support_ic_type == ODM_RTL8733B) {
			if (config_type == CONFIG_RF_RADIO) {
				if (e_rf_path == RF_PATH_A)
					READ_AND_CONFIG_MP(8733b, _radioa);
				else if (e_rf_path == RF_PATH_B)
					READ_AND_CONFIG_MP(8733b, _radiob);
			} else if (config_type == CONFIG_RF_TXPWR_LMT) {
				if (dm->rfe_type == 12)
					READ_AND_CONFIG_MP(8733b, _txpwr_lmt_type12);
				else
					READ_AND_CONFIG_MP(8733b, _txpwr_lmt);
			}
		}
#endif
#if (RTL8735B_SUPPORT)
...
/*@jj add 20170822*/
	...
	if (dm->support_ic_type == ODM_RTL8733B) {
		if (dm->en_tssi_mode)
			READ_AND_CONFIG_MP(8733b, _txpowertracktssi);
		else
			READ_AND_CONFIG_MP(8733b, _txpowertrack);
		READ_AND_CONFIG_MP(8733b, _txxtaltrack);
	}
#endif
...
/*@jj add 20170822*/
	...
	if (dm->support_ic_type == ODM_RTL8822C) {
		...
		if (dm->support_ic_type == ODM_RTL8733B) {
			if (config_type == CONFIG_BB_PHY_REG) {
				READ_AND_CONFIG_MP(8733b, _phy_reg);
			} else if (config_type == CONFIG_BB_AGC_TAB) {
				READ_AND_CONFIG_MP(8733b, _agc_tab);
			} else if (config_type == CONFIG_BB_PHY_REG_PG) {
				if (dm->rfe_type == 12)
					READ_AND_CONFIG(8733b, _phy_reg_pg_type12);
				else
					READ_AND_CONFIG(8733b, _phy_reg_pg);
			}
		}
#endif
#if (RTL8735B_SUPPORT)
...
#if (RTL8733B_SUPPORT)
	case ODM_RTL8733B:
		version = odm_get_version_mp_8733b_phy_reg();
		break;
#endif
#if (RTL8735B_SUPPORT)
	case ODM_RTL8735B:
