...
void rtl8733b_set_p2p_ps_offload_cmd(_adapter *padapter, u8 p2p_ps_state)
{
	...
	case P2P_PS_ENABLE:
		...
		if ((pwdinfo->opp_ps == 1) || (pwdinfo->noa_num > 0)) {
			/* rst p2p circuit */
			rtw_write8(padapter, REG_P2P_RST, BIT(0));
...
{
	struct sta_info *psta = rtw_tx_stat_get_ongoing_sta(Adapter, false);
...
{
	struct sta_info *psta = rtw_tx_stat_get_ongoing_sta(Adapter, true);
