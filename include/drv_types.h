...
#ifdef CONFIG_ECSA
#ifdef CONFIG_AP_MODE
#define IS_ECSA_WAIT_UPT_BCN(rfctl) ((rfctl)->ap_csa_wait_update_bcn == CSA_WAIT_UPT_BCN)
#define CHK_ECSA_STATE(adapter, state) check_ap_ecsa_state(adapter, state)
#else
#define IS_ECSA_WAIT_UPT_BCN(adapter) 0
#define CHK_ECSA_STATE(adapter, state) 0
#endif /* CONFIG_AP_MODE */
