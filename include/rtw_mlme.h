...
#define rtw_linked_check_alink(alink) rtw_linked_check((alink)->adapter) /* this driver has single link only */
...
/*
* 236 bytes fixed element
* 4 bytes magic cookie
* 3 bytes DHCP Message Type in options
* 1 bytes END in options
* Padding bytes, DHCP will fill 0x0 to the payload until the payload is 300 bytes.
*/
#define MINIMUM_DHCP_PAYLOAD_SIZE 300
...
#define GET_UDP_LEN(_udphdr) BE_BITS_TO_2BYTE(((u8 *)(_udphdr)) + 4, 0, 16)
#define GET_UDP_SIG1(_udppayload) BE_BITS_TO_1BYTE(((u8 *)(_udppayload)), 0, 8)
#define GET_UDP_SIG2(_udppayload) BE_BITS_TO_1BYTE(((u8 *)(_udppayload)) + 15, 0, 8)
#define IS_WAVEAGENT_PKT(_udppayload)             \
	(((GET_UDP_SIG1(_udppayload) == 0xcc) ||  \
	  (GET_UDP_SIG1(_udppayload) == 0xdd)) && \
	 (GET_UDP_SIG2(_udppayload) == 0xe2))
