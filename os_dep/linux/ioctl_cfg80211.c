...
skip_check_bss:
	...
	if (rtw_to_roam(padapter) > 0) {
		...
		if (check_fwstate(pmlmepriv, WIFI_MONITOR_STATE) != _TRUE) {
			rtw_cfg80211_connect_bss(pwdev, cur_network->network.MacAddress, pwdev_priv->bss
				, pmlmepriv->assoc_req + sizeof(struct rtw_ieee80211_hdr_3addr) + 2
				, pmlmepriv->assoc_req_len - sizeof(struct rtw_ieee80211_hdr_3addr) - 2
				, pmlmepriv->assoc_rsp + sizeof(struct rtw_ieee80211_hdr_3addr) + 6
				, pmlmepriv->assoc_rsp_len - sizeof(struct rtw_ieee80211_hdr_3addr) - 6
				...
				struct cfg80211_connect_params *sme)
{
	...
	RTW_INFO("privacy=%d, key=%p, key_len=%d, key_idx=%d, auth_type=%d, bss_type=%u\n",
		 sme->privacy, sme->key, sme->key_len, sme->key_idx, sme->auth_type,
		 pwdev_priv->rtw_wdev->conn_bss_type);
	...
	if (pwdev_priv->connect_req) {
		...
		pwdev_priv->bss = rtw_get_bss(adapter_to_wiphy(padapter), sme->channel,
					      sme->bssid, sme->ssid, sme->ssid_len,
					      Ndis802_11Infrastructure, sme->privacy);
		if (!pwdev_priv->bss)
			RTW_WARN(FUNC_NDEV_FMT ": fail to get bss reference!\n", FUNC_NDEV_ARG(ndev));
	} else {
		...
		struct cfg80211_ap_settings *settings)
{
	...
	if (cfg80211_ap_settings_to_ieee80211_channel(settings)) {
		...
		pbss_network->Configuration.DSConfig = chan->hw_value;
	}
#endif
...
#ifdef CONFIG_P2P
	type = rtw_p2p_check_frames_realloc_buf(padapter, &dump_buf, category, dump_len, _TRUE);
	if (type >= 0) {
		is_p2p = 1;
