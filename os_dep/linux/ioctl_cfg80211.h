...
#define rtw_get_bss(wiphy, ch, bssid, ssid, ssid_len, infra_mode, privacy) \
		cfg80211_get_bss(wiphy, ch, bssid, ssid, ssid_len, \
				 infra_mode == Ndis802_11Infrastructure ? IEEE80211_BSS_TYPE_ESS : IEEE80211_BSS_TYPE_IBSS, \
				 IEEE80211_PRIVACY(privacy))
#else
#define rtw_get_bss(wiphy, ch, bssid, ssid, ssid_len, infra_mode, privacy) \
		cfg80211_get_bss(wiphy, ch, bssid, ssid, ssid_len, \
				 infra_mode == Ndis802_11Infrastructure ? WLAN_CAPABILITY_ESS : WLAN_CAPABILITY_IBSS, \
				 infra_mode == Ndis802_11Infrastructure ? WLAN_CAPABILITY_ESS : WLAN_CAPABILITY_IBSS)
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0)
#define rtw_put_bss(wiphy, bss)	cfg80211_put_bss(wiphy, bss)
#else
#define rtw_put_bss(wiphy, bss)	cfg80211_put_bss(bss)
#endif
...
#define rtw_wdev_free_connect_req(rtw_wdev_data) \
	do { \
		...
		if ((rtw_wdev_data)->bss) { \
			rtw_put_bss((rtw_wdev_data)->rtw_wdev->wiphy, (rtw_wdev_data)->bss); \
			(rtw_wdev_data)->bss = NULL; \
		} \
	} while (0)
...
#define rtw_cfg80211_connect_bss(wdev, bssid, bss, req_ie, req_ie_len, resp_ie, resp_ie_len, status, gfp) cfg80211_connect_bss(wdev_to_ndev(wdev), bssid, bss, req_ie, req_ie_len, resp_ie, resp_ie_len, status, gfp, NL80211_TIMEOUT_UNSPECIFIED)
