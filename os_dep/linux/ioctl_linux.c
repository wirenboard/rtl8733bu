				  ...
				  union iwreq_data *wrqu, char *extra)
{
	...
	if ((wrqu->data.length <= 0) || (wrqu->data.length > MAX_IN_PATTERN_SIZE)) {
		...
		if ((strncmp(input, "pattern=", 8) == 0) ||(strncmp(input, "ack_pattern=", 12) == 0)) {
			...
			rtw_wow_pattern_sw_dump(NULL, padapter);
		} else {
...
/*
 * For user to set the mdns response packet content to the driver
 */
#ifdef CONFIG_MDNS_OFFLOAD
static int rtw_wowlan_set_mdns_resp(struct net_device *dev,
				    struct iw_request_info *info,
				    union iwreq_data *wrqu, char *extra)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	...
	input = (u8*)rtw_zvmalloc(MDNS_RESP_PROC_BUF_LEN);

	if (!input) {
		RTW_INFO("ERROR: alloc buffer failed\n");
		return -ENOMEM;
	}

	if ((wrqu->data.length <= 0) || (wrqu->data.length > MDNS_RESP_PROC_BUF_LEN)) {
		ret = -EFAULT;
		RTW_INFO("ERROR: parameter length error, len=%d\n", wrqu->data.length);
		goto _rtw_wowlan_exit;
	}
	...
	if (strncmp(input, "clean", 5) == 0 || strncmp(input, "clear", 5) == 0) {
	...
	num = sscanf(input, "%hhu %s", &resp_idx, mdns_resp);
...
_rtw_wowlan_exit:
	...
	if (input)
		rtw_vmfree(input, MDNS_RESP_PROC_BUF_LEN);
	return ret;
}

/*
 * For user to set the mdns response criteria to the driver
 * Format : "$id $type $name_offset"
 */

static int rtw_wowlan_set_mdns_match_criteria(struct net_device *dev,
					      struct iw_request_info *info,
					      union iwreq_data *wrqu, char *extra)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	...
	if ((wrqu->data.length <= 0) || (wrqu->data.length > sizeof(tmp))) {
		ret = -EFAULT;
		RTW_INFO("ERROR: parameter length error, len=%d\n", wrqu->data.length);
		goto _rtw_wowlan_exit;
	}
	...
	/* set pattern */
	if (copy_from_user(tmp, wrqu->data.pointer, wrqu->data.length))
		return -EFAULT;
	tmp[wrqu->data.length - 1] = '\0';
	/* leave PS first */
...
static int rtw_ext_set(struct net_device *dev,
		       struct iw_request_info *info,
		       union iwreq_data *wdata, char *extra)
{
	struct iw_point *wrqu = (struct iw_point *)wdata;
	...
	switch (subcmd) {
#ifdef CONFIG_MDNS_OFFLOAD
	case MDNS_OFFLOAD_RESP:
		RTW_INFO("set case MDNS_OFFLOAD_RESP\n");
		rtw_wowlan_set_mdns_resp(dev , info , wdata , extra);
		break;
	case MDNS_OFFLOAD_MATCH_CRIT:
		RTW_INFO("set case MDNS_OFFLOAD_MATCH_CRIT\n");
		rtw_wowlan_set_mdns_match_criteria(dev , info , wdata , extra);
		break;
#endif
	default:
		return -EIO;
...
static const struct iw_priv_args rtw_private_args[] = {
	...
	/* extset (use get type id, so don't require root privilege) */
	{ SIOCIWFIRSTPRIV + 0x19, IW_PRIV_TYPE_CHAR | 0x7FF, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK , ""},
 /* --- set/get sub-ioctls definitions --- */
...
#endif
/* --- extset sub-ioctls definitions.
 *Please ensure that the set_args and get_args are declared the same as extset.
 *Otherwise, iwpriv.c will not recognize them as sub-ioctls of the extset ioctl. --- */
#ifdef CONFIG_MDNS_OFFLOAD
	{ MDNS_OFFLOAD_RESP , IW_PRIV_TYPE_CHAR | 0x7FF, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "wow_mdns_resp" },
	{ MDNS_OFFLOAD_MATCH_CRIT , IW_PRIV_TYPE_CHAR | 0x7FF, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "wow_mdns_crit" },
#endif
};
