...
static bool io_records_ori_enabled;

static void *proc_start_io_records(struct seq_file *m, loff_t *pos)
{
	size_t seq = *pos;

	if (seq == 0) {
		io_records_ori_enabled = rtw_io_records_enabled();
		if (io_records_ori_enabled) {
			struct net_device *dev = m->private;
			_adapter *adapter = rtw_netdev_priv(dev);

			/* ori enabled, claim and disable */
			rtw_io_records_claim_and_enable(adapter_to_dvobj(adapter), false);
		}
	}

	if (!rtw_io_record_valid(seq))
		return NULL;
	return pos;
}

static void proc_stop_io_records(struct seq_file *m, void *v)
{
	if (!v) {
		if (io_records_ori_enabled && !rtw_io_records_enabled()) {
			struct net_device *dev = m->private;
			_adapter *adapter = rtw_netdev_priv(dev);

			/* ori enabled and disabled now, claim and enable */
			rtw_io_records_claim_and_enable(adapter_to_dvobj(adapter), true);
		}
	}
}

static void *proc_next_io_records(struct seq_file *m, void *v, loff_t *pos)
{
	size_t seq = ++(*pos);
...
static int proc_get_io_records(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	size_t seq = *((loff_t *)(v));
	...
	if (seq == 0)
		rtw_io_records_dump_title(m, tab);
	rtw_io_records_dump_value_by_seq(m, tab, seq);

	return 0;
}

static int proc_get_io_records_tab(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	size_t seq = *((loff_t *)(v));
...
static struct seq_operations seq_ops_io_records = {
	.start = proc_start_io_records,
	.stop  = proc_stop_io_records,
	.next  = proc_next_io_records,
	.show  = proc_get_io_records,
};

static struct seq_operations seq_ops_io_records_tab = {
	.start = proc_start_io_records,
	.stop  = proc_stop_io_records,
	.next  = proc_next_io_records,
	.show  = proc_get_io_records_tab,
};

static ssize_t proc_set_io_records(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	char tmp[32];
	int enable;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if (num >= 1) {
			...
			if (rtw_io_records_enabled() && !enable) {
				/* on -> off */
				rtw_io_records_claim_and_enable(dvobj, false);
			} else if (!rtw_io_records_enabled() && enable) {
				/* off -> on */
				rtw_io_records_clear();
				rtw_io_records_claim_and_enable(dvobj, true);
			}
		}
	}
...
static int proc_get_io_records_enabled(struct seq_file *m, void *v)
{
	RTW_PRINT_SEL(m, "%d\n", rtw_io_records_enabled());

	return 0;
}

static int proc_get_io_records_summary(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	bool ori_en = rtw_io_records_enabled();

	if (ori_en)
		rtw_io_records_claim_and_enable(dvobj, false);

	rtw_io_records_summary_dump(m);
...
static void _cmd_set(_adapter *padapter,
		     char *cmd_string, enum diag_cmd_type type)
{
	char buf[25] = {0};
	u32 cmd_len = 0;
	...
	if (cmd_len > 23) { /*23 + 1(space character) + 1(end character) = 25*/
		RTW_ERR("%s, cmd_len(%d) > 23, please check this!!\n",
			__func__, cmd_len);
	}
	/* Adding a whitespace at the end of the command string
	 * is a workaround for submodules string handling.
	 */
	...
	case DIAG_CMD_BB:
	case DIAG_CMD_RF:
		phydm_cmd(phydm, buf, cmd_len, 1, phydm_msg, PHYDM_MSG_LEN);
		break;
	case DIAG_CMD_MAC:
		rtw_halmac_cmd(adapter_to_dvobj(padapter),
			       buf, cmd_len, phydm_msg, PHYDM_MSG_LEN);
		break;
	default:
		RTW_ERR("%s, UNKNOW TYPE(%d)\n", __func__, type);
		break;
	}
}
...
static int proc_get_self_diag_info(struct seq_file *m, void *v)
{
	...
	/* MAC */
	_RTW_PRINT_SEL(m, "\n==================================[MAC info]==================================\n");
	_cmd_set(padapter, "self_diag", DIAG_CMD_MAC);
	_RTW_PRINT_SEL(m, "%s\n", phydm_msg);

	/* BB */
	_RTW_PRINT_SEL(m, "=================================[BB info]==================================\n");
	if (dvobj->bb_self_diag_en) {
		_cmd_set(padapter, "auto_dbg query 0", DIAG_CMD_BB);
		_RTW_PRINT_SEL(m, "%s\n", phydm_msg);

		_cmd_set(padapter, "auto_dbg query 2", DIAG_CMD_BB);
		_RTW_PRINT_SEL(m, "%s\n", phydm_msg);

		_cmd_set(padapter, "auto_dbg query 3", DIAG_CMD_BB);
		_RTW_PRINT_SEL(m, "%s\n", phydm_msg);
	...
	_cmd_set(padapter, "rf dz_dbg rfkinfo", DIAG_CMD_RF);
	_RTW_PRINT_SEL(m, "%s\n", phydm_msg);

	_cmd_set(padapter, "rf dz_dbg rtinfo", DIAG_CMD_RF);
	_RTW_PRINT_SEL(m, "%s\n", phydm_msg);
	...
	for (i = 0; i < RX_STATS_MAX_NUM; i++) {
		trx_submit = dvobj->trx_stats.rx_submit_cnt[i];
		trx_submit_fail = dvobj->trx_stats.rx_submit_fail[i];
		trx_complete = dvobj->trx_stats.rx_complete_cnt[i];
		trx_complete_fail = dvobj->trx_stats.rx_complete_fail[i];
		trx_last_submit = dvobj->trx_stats.last_rx_submit_cnt[i];
		strcpy(trx_string, "-------");
		...
		dvobj->trx_stats.last_rx_submit_cnt[i] = trx_submit;
	}
	...
	for (i = 0; i < TX_STATS_MAX_NUM; i++) {
		trx_submit = dvobj->trx_stats.tx_submit_cnt[i];
		trx_submit_fail = dvobj->trx_stats.tx_submit_fail[i];
		trx_complete = dvobj->trx_stats.tx_complete_cnt[i];
		trx_complete_fail = dvobj->trx_stats.tx_complete_fail[i];
		trx_last_submit = dvobj->trx_stats.last_tx_submit_cnt[i];
		strcpy(trx_string, "-------");
		...
		dvobj->trx_stats.last_tx_submit_cnt[i] = trx_submit;
	}
#endif
	return 0;
}
...
static ssize_t proc_set_self_diag_info(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (strcmp(type, "bb") == 0) {
		...
		_cmd_set(adapter, buf, DIAG_CMD_BB);
...
#endif /* CONFIG_SDIO_HCI */
#ifdef CONFIG_RTW_IO_RECORDS
	RTW_PROC_HDL_SEQ("io_records", &seq_ops_io_records, proc_set_io_records),
	RTW_PROC_HDL_SEQ("io_records_tab", &seq_ops_io_records_tab, proc_set_io_records),
	RTW_PROC_HDL_SSEQ("io_records_enabled", proc_get_io_records_enabled, NULL),
	RTW_PROC_HDL_SSEQ("io_records_summary", proc_get_io_records_summary, NULL),
#endif
...
#endif
	RTW_PROC_HDL_SSEQ("igi_offset", proc_get_igi_offset, proc_set_igi_offset),
};
